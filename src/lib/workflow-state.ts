...
function normalizeInputsState(value: unknown): InputsFormState {
  if (Array.isArray(value)) {
    return {
      ...
      integrations: value.filter(isRecord).map((entry, index) => ({
        ...
        sensitivity: entry.dataClass === "INTERNAL" || entry.dataClass === "CONFIDENTIAL" || entry.dataClass === "FINANCIAL" || entry.dataClass === "AUDIT" || entry.dataClass === "PII" ? entry.dataClass : "INTERNAL",
        thirdParty: false,
      })),
    };
  }
  ...
  const dataEntries = Array.isArray(value.dataEntries)
    ? value.dataEntries.filter(isRecord).map((entry, index) => ({
        ...
        classification: entry.classification === "INTERNAL" || entry.classification === "CONFIDENTIAL" || entry.classification === "FINANCIAL" || entry.classification === "AUDIT" || entry.classification === "PII" ? entry.classification : "INTERNAL",
  ...
  const integrations = Array.isArray(value.integrations)
    ? value.integrations.filter(isRecord).map((entry, index) => ({
        ...
        sensitivity: entry.sensitivity === "INTERNAL" || entry.sensitivity === "CONFIDENTIAL" || entry.sensitivity === "FINANCIAL" || entry.sensitivity === "AUDIT" || entry.sensitivity === "PII" ? entry.sensitivity : "INTERNAL",
  ...
  return {
    risks: Array.isArray(value.risks)
      ? value.risks.filter(isRecord).map((risk, index) => ({
          ...
          severity: risk.severity === "Critical" || risk.severity === "High" || risk.severity === "Medium" || risk.severity === "Low" ? risk.severity : "Low",
          ...
          status: risk.status === "Open" || risk.status === "Accepted" || risk.status === "Mitigated" || risk.status === "Closed" ? risk.status : "Open",
...
export function createWorkflowSnapshot(value?: Partial<WorkflowSnapshot>): WorkflowSnapshot {
  return {
    intake: isRecord(value?.intake) ? (value.intake as Partial<RequirementIntake>) : {},
