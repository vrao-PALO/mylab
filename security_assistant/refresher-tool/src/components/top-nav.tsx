...
export function TopNav() {
  ...
  const [visited, setVisited] = useState<Set<string>>(() => (pathname ? new Set([pathname]) : new Set()));
  ...
  useEffect(() => {
    ...
    const timerId = window.setTimeout(() => {
      const nextVisited = loadVisited();
      nextVisited.add(pathname);
  ...
  const visitedSteps = WORKFLOW_STEPS.filter((step) => visited.has(step.href)).length;
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-3">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-base font-semibold leading-tight text-[#10203d]">Audit Readiness Workflow</h1>
          </div>
          <div className="flex items-center gap-3">
            <RoleModeSelector />
            <button onClick={() => setMobileOpen(!mobileOpen)} className="rounded-md border border-[#c9d5e6] px-3 py-1.5 text-xs text-[#2a4a80] lg:hidden">
        ...
        <div className="mt-3">
          ...
          <div className="flex gap-0.5">
            {WORKFLOW_STEPS.map((step) => {
              const isVisited = visited.has(step.href);
              ...
              return (
                <Link
                  ...
                  href={step.href}
                  title={step.label}
                  className={`h-1.5 flex-1 rounded-full transition-colors ${isCurrent ? "bg-[#1f4f97]" : isVisited ? "bg-[#7aa4d8]" : "bg-[#e1e8f3]"}`}
                />
              );
            })}
          </div>
          <div className="mt-1 hidden gap-0.5 lg:flex">
            {WORKFLOW_STEPS.map((step) => {
              const isVisited = visited.has(step.href);
              const isCurrent = pathname === step.href;
              return (
                <Link
                  key={step.href}
                  href={step.href}
                  className={`flex-1 truncate rounded px-0.5 py-1 text-center text-[10px] leading-none transition-colors ${isCurrent ? "font-bold text-[#1f4f97]" : isVisited ? "text-[#4a7ab0]" : "text-[#a0b4cc]"}`}
                >
          ...
          {OTHER_LINKS.map((item) => {
            ...
            return (
              <Link
                ...
                className={`rounded-md border px-2.5 py-1 text-xs transition-colors ${isCurrent ? "border-[#1f4f97] bg-[#1f4f97] text-white" : "border-[#b9c6da] bg-white text-[#1b355f] hover:bg-[#e9f0fb]"}`}
              >
                {item.label}
              </Link>
            );
