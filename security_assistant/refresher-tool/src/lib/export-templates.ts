...
function describeBaseline(version: number | undefined, createdAt: string | undefined, fallback = "Not captured") {
  if (!version || !createdAt) return fallback;
  return `v${version} (${createdAt.split("T")[0]})`;
}
...
export function generateEngagementSummary(context: WorkflowExportContext) {
  ...
  const evidenceCoverage = context.evidence.length
    ...
    : "0/0";
  const latestIntakeBaseline = context.baselines.intake.at(-1);
  const latestScopeBaseline = context.baselines.scope.at(-1);
  ...
  return [
    ...
    `- Intake baseline: ${describeBaseline(latestIntakeBaseline?.version, latestIntakeBaseline?.createdAt)}`,
    `- Scope baseline: ${describeBaseline(latestScopeBaseline?.version, latestScopeBaseline?.createdAt)}`,
    `- Scope changes after baseline: ${Math.max(context.baselines.scope.length - 1, 0)}`,
    "",
    "## Scope",
