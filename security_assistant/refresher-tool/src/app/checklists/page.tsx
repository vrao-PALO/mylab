...
import { type ChecklistIndustry, type ChecklistState } from "@/lib/workflow-state";
...
const STAGES: Stage[] = [
  {
    ...
    tasks: [
      ...
      { id: "s1-t2", text: "Identify industry sector and applicable framework overlay (FI to MAS TRM; GovTech to IM8 and PDPA; Generic to OWASP and CIS)", industries: ["All"] },
      ...
      { id: "s1-t6", text: "Confirm citizen data handling and GovTech or IM8 obligations", industries: ["GovTech"] },
    ],
  ...
  },
  {
    ...
    assurance: ["Architecture diagram reviewed and current", "Trust boundary list documented", "Prod or non-prod isolation confirmed"],
  },
  {
    ...
    tasks: [
      { id: "s3-t1", text: "Apply the requirement derivation chain: business function to sensitive data to threat exposure to security requirement to design control to verification to residual risk", industries: ["All"] },
  ...
  },
  {
    ...
    tasks: [
      ...
      { id: "s5-t2", text: "SAST or SCA scans integrated into CI/CD and baseline established", industries: ["All"] },
      { id: "s5-t3", text: "VAPT scoped and executed - web, API, mobile (if applicable), cloud config", industries: ["All"] },
      { id: "s5-t4", text: "VAPT findings triaged: Critical and High findings remediated and retested before go-live", industries: ["All"], pitfall: "Open critical or high findings at go-live is a production blocker - MAS TRM is explicit on this." },
    ...
    questions: ["Has threat modelling been done and documented?", "Are SAST or SCA scans passing in CI/CD?", "Have all VAPT critical or high findings been remediated and retested?", "Have logs been reviewed and validated?"],
    assurance: ["Threat model documented", "VAPT report produced and remediation tracked", "SAST or SCA baseline in CI/CD", "Logging validation record produced", "Access review completed"],
  },
  {
    ...
    tasks: [
      { id: "s6-t1", text: "Compile all open design gaps with severity (Critical to Low) and assigned owner", industries: ["All"] },
...
export default function ChecklistsPage() {
  const role = useRole();
  const [state, setState] = useFormPersistence<ChecklistState>("checklists", DEFAULT_STATE);

  const visibleItems = (items: CheckItem[]) =>
    items.filter((item) => state.industry === "All" || item.industries.includes("All") || item.industries.includes(state.industry));

  const totalItems = STAGES.flatMap((stage) => visibleItems(stage.tasks)).length;
  const completedItems = STAGES.flatMap((stage) => visibleItems(stage.tasks)).filter((item) => state.checked[item.id]).length;
  const progressPct = totalItems > 0 ? Math.round((completedItems / totalItems) * 100) : 0;
  const showAssurance = role !== "Presales";
  ...
  return (
    <section className="space-y-5">
      <header className="rounded-xl border border-[#c9d5e6] bg-white p-6">
        ...
        <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-[#e1e8f3]">
        ...
        <div className="mt-3 rounded-md border border-[#d8e0ed] bg-[#f8fbff] px-4 py-3 text-xs text-[#33496f]">
          ...
          {role === "Architect"
            ? "Use the checklist as the structured walkthrough for design, assurance, and sign-off readiness."
            : role === "Auditor"
              ? "Keep the evidence-oriented items visible so another reviewer can follow the audit trail stage by stage."
              : "Presales mode keeps the checklist discussion-oriented. Detailed assurance expectations are hidden until the engagement moves into delivery or audit."}
        </div>
        <div className="mt-3 flex gap-2">
          {(["All", "FI", "GovTech", "Generic"] as ChecklistIndustry[]).map((industry) => (
            <button
              key={industry}
              type="button"
              onClick={() => setState({ ...state, industry })}
              className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors ${state.industry === industry ? "bg-[#1f4f97] text-white border-[#1f4f97]" : "bg-[#f8fbff] text-[#2a4a80] border-[#c9d5e6] hover:border-[#1f4f97]"}`}
            >
              {industry}
            </button>
          ))}
      ...
      {STAGES.map((stage) => {
        ...
        const stageComplete = items.length > 0 && items.every((item) => state.checked[item.id]);
        const stageCount = items.filter((item) => state.checked[item.id]).length;
        return (
          <article key={stage.id} className={`rounded-xl border bg-white ${stageComplete ? "border-green-300" : "border-[#c9d5e6]"}`}>
            <button type="button" onClick={() => toggleStage(stage.id)} className="w-full p-5 text-left">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-sm text-[#3a5480]">{state.expanded === stage.id ? "?" : "?"}</span>
              </div>
            </button>

            {state.expanded === stage.id && (
              <div className="space-y-5 border-t border-[#e1e8f3] px-5 pb-5 pt-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {items.map((item) => (
                      <label key={item.id} className="group flex cursor-pointer gap-3">
                        <input type="checkbox" checked={Boolean(state.checked[item.id])} onChange={() => toggle(item.id)} className="mt-0.5 h-4 w-4 rounded border-[#c9d5e6] accent-[#1f4f97]" />
                        <div>
                          <span className={`text-sm ${state.checked[item.id] ? "line-through text-[#8fa3c2]" : "text-[#273f67]"}`}>{item.text}</span>
                          {item.pitfall && <p className="mt-0.5 text-xs text-amber-600">Pitfall: {item.pitfall}</p>}
                          {showAssurance && item.evidence && <p className="mt-0.5 text-xs italic text-[#4a6080]">Evidence: {item.evidence}</p>}
                        </div>
                      </label>
                    ))}
                  </div>
                ...
                <div className={`grid gap-4 ${showAssurance ? "md:grid-cols-2" : "md:grid-cols-1"}`}>
                  <div>
                    ...
                    <ul className="space-y-1">
                      {stage.questions.map((question, index) => (
                        <li key={index} className="flex gap-2 text-sm text-[#273f67]">
                          <span className="shrink-0 text-[#1f4f97]">?</span>
                          <span>{question}</span>
                        </li>
                      ))}
                    </ul>
                  ...
                  {showAssurance && (
                    <div>
                      <h4 className="mb-2 text-xs font-semibold uppercase tracking-[0.1em] text-[#3a5480]">Required assurance and evidence</h4>
                      <ul className="space-y-1">
                        {stage.assurance.map((assurance, index) => (
                          <li key={index} className="flex gap-2 text-sm text-[#273f67]">
