...
function getSuggestedTitle() {
  ...
  return [workflow.intake.engagementType, workflow.intake.industry, workflow.intake.timeline].filter(Boolean).join(" - ");
}
...
export default function EngagementPackagePage() {
  ...
  const [currentWorkflow, setCurrentWorkflow] = useState(readCurrentWorkflow);
    ...
    <section className="space-y-5">
      ...
      <article className="rounded-xl border border-[#1f4f97] bg-white p-5">
        ...
        <div className="mb-3 grid gap-3 md:grid-cols-2">
          <div className="rounded-md border border-[#d8e0ed] bg-[#f8fbff] p-3 text-xs text-[#33496f]">
            <p className="font-semibold text-[#15305a]">Intake baseline</p>
            <p className="mt-1">{currentWorkflow.baselines.intake.at(-1) ? `v${currentWorkflow.baselines.intake.at(-1)?.version}` : "Not captured"}</p>
          </div>
          <div className="rounded-md border border-[#d8e0ed] bg-[#f8fbff] p-3 text-xs text-[#33496f]">
            <p className="font-semibold text-[#15305a]">Scope baseline</p>
            <p className="mt-1">{currentWorkflow.baselines.scope.at(-1) ? `v${currentWorkflow.baselines.scope.at(-1)?.version}` : "Not captured"}</p>
          </div>
        </div>
        <div className="flex gap-3">
          ...
          <button type="button" onClick={saveEngagement} disabled={saving || !title.trim()} className="rounded-md bg-[#1f4f97] px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-[#183d7a] disabled:opacity-50">
            {saving ? "Saving..." : "Save"}
          </button>
        </div>
      ...
      <article className="overflow-hidden rounded-xl border border-[#c9d5e6] bg-white">
        <div className="flex items-center justify-between border-b border-[#e4eaf4] bg-[#f8fbff] px-5 py-3">
          ...
          <button type="button" onClick={() => void fetchEngagements()} className="text-xs text-[#6a8ab0] transition-colors hover:text-[#1f4f97]">Refresh</button>
        </div>
              ...
              <div key={engagement.id} className="space-y-2 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex shrink-0 flex-wrap gap-2">
                    <button type="button" onClick={() => void loadEngagement(engagement.id)} className="rounded-md border border-[#1f4f97] bg-[#edf3fc] px-3 py-1.5 text-xs font-medium text-[#1f4f97] transition-colors hover:bg-[#1f4f97] hover:text-white">
                      {loadedId === engagement.id ? "Loaded" : "Load"}
                    </button>
                    <select value={engagement.status} onChange={(event) => void updateStatus(engagement.id, event.target.value)} className="rounded-md border border-[#c9d5e6] bg-[#f8fbff] px-2 py-1 text-xs text-[#2a4a80] focus:outline-none focus:ring-1 focus:ring-[#1f4f97]">
      ...
      <div className="rounded-xl border border-[#c9d5e6] bg-[#f8fbff] p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {[["/intake", "Intake"], ["/scope", "Scope"], ["/inputs", "Inputs"], ["/control-mapping", "Controls"], ["/risk-exceptions", "Risk Register"], ["/audit-evidence", "Evidence Tracker"]].map(([href, label]) => (
            <Link key={href} href={href} className="rounded-md border border-[#c9d5e6] bg-white px-3 py-1 text-xs text-[#1f4f97] transition-colors hover:bg-[#edf3fc]">
              {label}
            </Link>
          ))}
        </div>
