...
export default function GenericIndustryPage() {
  ...
  const filtered = PRESALES_ITEMS.filter(
    (item) => clientType === "All" || item.clientTypes.includes("All") || item.clientTypes.includes(clientType),
  );
    ...
    <section className="space-y-5">
      ...
      <article className="rounded-xl border border-[#c9d5e6] bg-white p-5">
        ...
        <ul className="space-y-1">
          {DISCOVERY_PROMPTS.map((prompt, index) => (
            <li key={index} className="flex gap-2 text-sm text-[#273f67]">
              <span className="shrink-0 font-bold text-[#1f4f97]">&gt;</span><span>{prompt}</span>
            </li>
          ))}
        </ul>
      </article>
      ...
      <div className="flex flex-wrap gap-1">
        {(Object.keys(CLIENT_LABELS) as ClientType[]).map((client) => (
          <button
            key={client}
            onClick={() => setClientType(client)}
            className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors ${clientType === client ? "border-[#1f4f97] bg-[#1f4f97] text-white" : "border-[#c9d5e6] bg-[#f8fbff] text-[#2a4a80] hover:border-[#1f4f97]"}`}
          >
      ...
      <div className="space-y-3">
        {filtered.map((item) => (
          <article key={item.id} className={`overflow-hidden rounded-xl border bg-white ${item.redFlag ? "border-red-300" : "border-[#c9d5e6]"}`}>
            <button
              className="flex w-full items-start justify-between p-5 text-left transition-colors hover:bg-[#f8fbff]"
              onClick={() => setExpanded(expanded === item.id ? null : item.id)}
            >
              <div className="flex-1 pr-4">
                <div className="mb-1 flex flex-wrap gap-2">
                  {item.redFlag && <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs font-semibold text-red-700">! Escalate to architect</span>}
              ...
              <span className="shrink-0 text-sm text-[#3a5480]">{expanded === item.id ? "^" : "v"}</span>
            </button>

            {expanded === item.id && (
              <div className="space-y-3 border-t border-[#e1e8f3] p-5">
                <div className="rounded-md border border-green-200 bg-green-50 p-3">
                  <p className="mb-1 text-xs font-semibold uppercase text-green-700">Guidance</p>
                  <p className="text-sm text-[#1a3a1a]">{item.guidance}</p>
                </div>
                <div className="flex flex-wrap gap-1">
                  {item.frameworks.map((frameworkId) => (
                    <span key={frameworkId} className="rounded-full border border-[#c9d5e6] bg-[#edf3fc] px-2 py-0.5 text-xs text-[#1e3d6c]">
                      {frameworkId.toUpperCase().replace(/-/g, " ")}
                    </span>
                  ))}
                </div>
              </div>
      ...
      <article className="rounded-xl border border-[#c9d5e6] bg-white p-5">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {[
            ...
            <div key={column.label} className={`rounded-md border p-3 ${column.color}`}>
              <p className="mb-2 text-xs font-semibold text-[#10203d]">{column.label}</p>
              <ul className="space-y-1">
                {column.items.map((item, index) => (
                  <li key={index} className="flex gap-1 text-xs text-[#273f67]"><span>*</span><span>{item}</span></li>
                ))}
              </ul>
            </div>
          ))}
